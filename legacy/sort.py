...
- Selects a cross-section via the shortest tube's midpoint & a plane normal from local vectors.
...
def infer_pixel_size_A(df_in: pd.DataFrame, override: float = None) -> float:
    """Return pixel size (Å/px). Uses rlnImagePixelSize column; otherwise uses override."""
    if "rlnImagePixelSize" in df_in.columns:
        # Convert column to numeric and return first non-NaN value
        val = pd.to_numeric(df_in["rlnImagePixelSize"], errors="coerce").dropna()
        if not val.empty:
            return float(val.iloc[0])
    if override is not None:
        return float(override)

    raise ValueError("Pixel size not found. Provide --angpix or include rlnImagePixelSize in STAR.")
...
def find_cross_section_points(data, plane_normal, reference_point):
    """
    For each tube, pick the point closest to the plane; also return the max min-distance.
    Expects columns rlnHelicalTubeID, rlnCoordinateX/Y/Z (IN PIXELS).
    """
    cross_section = []
    max_distance = 0
    grouped = data.groupby('rlnHelicalTubeID', sort=False)
    for tube_id, group in grouped:
...
def calculate_normal_vector(tube_points, window_size=3):
    ...
    vecs = []
    for i in range(s, e):
        vecs.append(tube_points[i + 1] - tube_points[i])
...
def process_cross_section(data):
    """
    Find cross-section based on the midpoint of the SHORTEST tube.
    """
    shortest_id, midpoint = find_shortest_tube(data)
    tube_pts = data.loc[data['rlnHelicalTubeID'] == shortest_id, ['rlnCoordinateX','rlnCoordinateY','rlnCoordinateZ']].to_numpy(float)
...
def detect_multiple_missing_points(rot_angles, gap_threshold=50):
    """
    Find large angular gaps in rot angles → indicate missing tubes.
...
def get_tube_order_from_rot(rotated_cross_section):
...
def plot_cs(cross_section_px: pd.DataFrame, pixel_size_A: float, output_png: str = None):
    ...
    plt.xlabel('X (nm)')
    plt.ylabel('Y (nm)')
    plt.title('Tube Plot: X vs Y with Tube IDs and Circular Connecting Lines')
...
def run_pipeline(df: pd.DataFrame, pixel_size_A: float,
    ...
    If there are more than 9 tubes, returns input unchanged (no sorting).
    """
    ...
    n_tubes = df['rlnHelicalTubeID'].nunique()
    if n_tubes > 9:
        print(f"[WARNING] Found {n_tubes} tubes (> 9). Skipping sorting and returning input unchanged.")
    ...
    order_ids = get_tube_order_from_rot(cs_for_order) if 'rlnAngleRot' in cs_for_order.columns else cs_for_order['rlnHelicalTubeID'].tolist()
    df_out = df.copy()
    cs_out = cs_with_rot.copy()

    if renumber and len(order_ids) > 0:
        df_out, cs_out, mapping = renumber_tube_ids(df_out, order_ids, cs_out)
...
def main():
    ...
    starfile.write(df_out, args.output, overwrite=True)
